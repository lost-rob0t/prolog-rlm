...
test(subagent_records_trusted_delegation_provenance,
    ...
    setup_call_cleanup(
        ...
        ( agent_spawn(Runtime, none, agent_spec(parent), Caps, ok(Parent)),
          ...
          agent_status(Runtime, Child, ok(ChildStatus)),
          ChildDelegation = ChildStatus.spec.metadata.delegation,
          assertion(ChildDelegation.role == reviewer),
          assertion(ChildDelegation.skills == ['rlm-facts']),
          assertion(ChildDelegation.source == trusted_host),
