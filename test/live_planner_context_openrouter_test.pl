...
run_live_planner_context_case(Registry) :-
    Query = "Perform a repository self-audit before returning a final result. Retrieve the opaque context input, then use the active project_read tool to inspect all three authoritative records: AGENTS.md, docs/spec-mode.md, and rage/277-planner-protocol-context.org. Do not guess their contents and do not omit or substitute any record.",
    Context = text("RLM_SELF_AUDIT_CONTEXT_OK: inspect the repository instructions, SPEC contract, and issue design record before finalizing."),
...
validate_live_planner_context_result(Result) :-
    ...
    sort(Paths0, Paths),
    assertion(Paths == ["AGENTS.md",
                        "docs/spec-mode.md",
                        "rage/277-planner-protocol-context.org"]),
    member(AgentsEnvelope, ToolEnvelopes),
    AgentsEnvelope.value.path == "AGENTS.md",
    assertion(sub_string(AgentsEnvelope.value.content, _, _, _,
                         "ADR/ADARD/research/design record")),
    member(SpecEnvelope, ToolEnvelopes),
    SpecEnvelope.value.path == "docs/spec-mode.md",
    assertion(sub_string(SpecEnvelope.value.content, _, _, _,
                         "requirements -> SPEC source -> canonical Spec -> validate -> freeze")),
    member(DesignEnvelope, ToolEnvelopes),
    DesignEnvelope.value.path == "rage/277-planner-protocol-context.org",
    assertion(sub_string(DesignEnvelope.value.content, _, _, _,
...
log_live_planner_context_evidence(Result) :-
    ...
    format('planner_context_spec_adard_invariants_checked: true~n', []),
