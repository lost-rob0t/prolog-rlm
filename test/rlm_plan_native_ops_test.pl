...
/* D6-11 plan-native deterministic mutations (recorded verbatim in
   ...
   reconciliation does: the canonical desugared form
   ...
   The plan_graph_run tests at the end drive the SAME canonical handler
   wiring through the plan-graph executor's native_handlers/1 option,
   proving the executor path inherits the #57/#53 durable effect
   boundary and authority semantics while the executor itself stays a
   pure scheduler with no effect path of its own. */
...
/* Executor-level canonical wiring (#293 reconciliation): the SAME host
   closure routing through the canonical rlm_tool registry is what the
   plan-graph executor invokes through native_handlers/1. The plan-graph
   path therefore inherits the #57/#53 durable effect boundary and
   authority semantics: the executor stays a pure scheduler, performs no
   effects of its own, and the canonical boundary owns admission,
   dispatch, and observation. */

/* Single-step JSON graph over a plan-native op, exactly what the
   executor parses and desugars. */
plan_native_graph(sync_remote,
                  "{\"steps\":[{\"id\":\"n1\",\"op\":\"sync_remote\",\"args\":{\"op\":\"push\"},\"bind\":\"a\"}]}").
plan_native_graph(index,
                  "{\"steps\":[{\"id\":\"n1\",\"op\":\"index\",\"args\":{\"scope\":\"all\"},\"bind\":\"a\"}]}").
plan_native_graph(delete,
                  "{\"steps\":[{\"id\":\"n1\",\"op\":\"delete\",\"args\":{\"path\":\"a.py\"},\"bind\":\"a\"}]}").
plan_native_graph(run,
                  "{\"steps\":[{\"id\":\"n1\",\"op\":\"run\",\"args\":{\"command\":\"sync\"},\"bind\":\"a\"}]}").

plan_native_executor_handler(Context, Registry, Op,
                             native_handler(Op,
                                 plan_native_ops_test:native_plan_handler(
                                     Context, Registry, Op))).

run_native_graph(Json, Caps, Handler, ExpectedValue, _Outcome) :-
    plan_graph_run(Json, Caps, [native_handlers([Handler])], inputs{},
                   RunOutcome),
    RunOutcome = ok(Result),
    assertion(Result.status == completed),
    get_dict(state, Result, GraphState),
    get_dict(results, GraphState, ResultsMap),
    get_dict(n1, ResultsMap, StepResult),
    get_dict(outcome, StepResult, PlanResult),
    get_dict(value, PlanResult, BoundValue),
    assertion(BoundValue == ExpectedValue).

/* 5. Executor-level admitted effect: the plan-native step dispatches
   through the canonical boundary on the plan-graph path — exactly one
   durable attempt carrying the content-derived normalized fingerprint;
   an identical re-run is a replay with no second external dispatch and
   no second durable attempt. */
test(plan_graph_run_native_admitted_effect_observed) :-
    setup_call_cleanup(
        setup_effect_store(_),
        ( dangerous_context(Context),
          native_registry(Registry),
          plan_native_graph(sync_remote, Json),
          plan_native_executor_handler(Context, Registry, sync_remote,
                                       Handler),
          run_native_graph(Json, [tool(sync_remote)], Handler,
                           ok(json{synced:push}), _),
          native_dispatch_count(sync_remote, OneDispatch),
          assertion(OneDispatch =:= 1),
          rlm_effect_attempts(_, _, [Attempt]),
          assertion(Attempt.status == observed),
          get_dict(fingerprint, Attempt, Fingerprint),
          ...
          run_native_graph(Json, [tool(sync_remote)], Handler,
                           ok(json{synced:push}), _),
          native_dispatch_count(sync_remote, StillOneDispatch),
          assertion(StillOneDispatch =:= 1),
          rlm_effect_attempts(_, _, [SameAttempt]),
          assertion(SameAttempt.fingerprint == Fingerprint),
          cleanup_context(Context),
...
/* 6. Executor-level capability fail-closed: an ungranted tool(Op) is a
   validation error before any dispatch, and the external adapter
   counter proves no effect ran. */
test(plan_graph_run_native_capability_denied_fails_closed,
     [forall(plan_native_op_case(Op, _Args))]) :-
    setup_call_cleanup(
        setup_effect_store(_),
        ( dangerous_context(Context),
          native_registry(Registry),
          plan_native_graph(Op, Json),
          plan_native_executor_handler(Context, Registry, Op, Handler),
          plan_graph_run(Json, [tool(other)],
                         [native_handlers([Handler])], inputs{},
                         error(Error)),
...
/* 7. Executor-level observation op: index never enters the durable
   effect store and re-executes fresh on every graph run. */
test(plan_graph_run_native_observation_op_reexecutes_fresh) :-
    setup_call_cleanup(
        setup_effect_store(_),
        ( dangerous_context(Context),
          native_registry(Registry),
          plan_native_graph(index, Json),
          plan_native_executor_handler(Context, Registry, index, Handler),
          run_native_graph(Json, [tool(index)], Handler,
                           ok(json{indexed:all}), _),
          run_native_graph(Json, [tool(index)], Handler,
                           ok(json{indexed:all}), _),
          native_dispatch_count(index, TwoFreshDispatches),
          assertion(TwoFreshDispatches =:= 2),
