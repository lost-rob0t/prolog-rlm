...
test(skill_packages_use_canonical_skill_metadata_and_graph,
    ...
    once((member(Node, Graph.nodes), Node.unit == skill('review-pr'))),
    assertion(Node.category == review),
    once((member(Edge, Graph.edges),
          Edge.kind == suggests,
          Edge.from == skill('review-pr'),
          Edge.to == skill(helper))),
...
test(default_prolog_rlm_core_skills_join_same_graph) :-
    ...
    agent_zero_context_compile(Request, ok(Result)),
    Graph = Result.skill_graph,
    once((member(CoreNode, Graph.nodes),
          CoreNode.unit == skill('rlm-operate'))),
