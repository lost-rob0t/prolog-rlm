...
test(skill_packages_use_canonical_skill_metadata_and_graph,
    ...
    get_dict(skill_graph, Result, Graph),
    get_dict(nodes, Graph, Nodes),
    get_dict(edges, Graph, Edges),
    once((member(Node, Nodes),
          get_dict(unit, Node, skill('review-pr')))),
    get_dict(category, Node, Category),
    assertion(Category == review),
    once((member(Edge, Edges),
          get_dict(kind, Edge, suggests),
          get_dict(from, Edge, skill('review-pr')),
          get_dict(to, Edge, skill(helper)))),
    assertion(\+ (member(HiddenNode, Nodes),
                  get_dict(unit, HiddenNode, skill('hidden-skill')))),
...
test(default_prolog_rlm_core_skills_join_same_graph) :-
    ...
    get_dict(skill_graph, Result, Graph),
    get_dict(nodes, Graph, Nodes),
    once((member(CoreNode, Nodes),
          get_dict(unit, CoreNode, skill('rlm-operate')))),
    get_dict(source, CoreNode, Source),
    assertion(Source == prolog_rlm_core),
