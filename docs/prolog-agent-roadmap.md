...
- the versioned CST/query/semantic/freshness layers from #96-#99 that turn the landed #94/#95 parser and registry substrate into the actual project parser/indexer and canonical project KB; #96-#98 are now substrate (versioned CST, grouped query captures, and the `rlm_project_semantic` normalized-facts layer with provenance, bounded resolution, and the #288-compatible symbol index), leaving #99 freshness/invalidation as the open layer;
...
The direct Tree-sitter mechanics from #94, the declarative Project/File/Language/grammar registry from #95, the bounded versioned CST observations from #96, the grouped #97 query/capture layer, and the #98 `rlm_project_semantic` normalized-facts layer are now substrate. The #96 layer handles both code and structured non-code project files, carries exact parse/grammar/source provenance, and keeps stale or partial generations explicit; #98 turns captures into closed semantic observations (definitions, references, calls, imports, exports) with exact provenance, bounded resolution with explicit resolved/unresolved/ambiguous/external states, and a durable direct-observation journal, and refuses mixed/stale generations through #97 currentness. Implement #99 behind the same semantic project-state boundary established by `rlm_spec`/`rlm_verify`: enforce incremental freshness and coherent snapshots over the landed layers. Source, build metadata, package metadata, and configuration should be parsed once into canonical project knowledge. Planner and verifier consumers query that knowledge through trusted semantic providers. Do not add a planner-only parser and a verifier-only parser.
...
- #70: project context should incorporate canonical project-KB snapshot references and provenance; #94 supplies direct Tree-sitter parser mechanics, #95 Project/File/Language/grammar selection, #96/#97 versioned CST and structural-query observations, and #98 supplies the normalized semantic-facts layer with the bounded resolver and symbol index, while #99 still needs coherent snapshot/freshness layers.
...
- #93/#96-#99: #94/#95 are the direct parser and declarative source-registry substrate, #96/#97 provide versioned CST/query observations, and #98 now provides normalized semantic project knowledge with provenance, bounded resolution, and the #288-compatible symbol index; #99 remains required before the canonical project parser/indexer and project KB are complete.
