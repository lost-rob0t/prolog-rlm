...
- deterministic Prolog-owned `SKILL.md` discovery/activation with explicit-only handling, dependency closure, prompt ceilings, explainable decisions, lazy body loading, and optional injection into the root completion planner;
...
The generic durable effect substrate is already on `main` through #78, #83, and #85, and the first effectful tool path from #86 is now adopted. Before repository mutation is a normal product feature, finish the **remaining canonical adoption** work tracked by #79.
...
The first symbolic prompt-compiler slice is now concrete: Prolog can select `SKILL.md` instructions before the planner call and account for their own bounded prompt cost without making the model decide which skills exist. Extend that same compiler ledger to project instructions, tool schemas, MCP metadata, rendering overhead, and the managed rolling-context budget rather than growing parallel prompt-assembly paths.
...
- #101/#117: managed-context accounting still needs tools, MCP, project instructions, and rendering overhead to join the same compiler ledger; #117 supplies the first concrete Prolog-owned skill-input implementation.
