...
Deterministic CI covers allow/deny behavior, pre-invocation denial, narrowing, malformed arguments, numeric schema bounds, timeout, oversized output, project-root confinement, discovery, and typed-plan execution. Trusted same-repository CI additionally asks a real OpenRouter root planner, without a fixed plan or `planner_instruction`, to retrieve opaque context and execute the actual registered `project_read` tool. The live assertions use runtime transitions and independently inspect both bound evidence values rather than trusting model prose.
