...
block_targets(Blocks, Targets) :-
    findall(Target, member(org_source_block{target:Target}, Blocks), Targets0),
    list_to_set(Targets0, Targets).

render_target(Blocks, Target, Output) :-
    findall(Body,
            member(org_source_block{target:Target, body:Body}, Blocks),
            Bodies),
    atomics_to_string(Bodies, "", Content),
    findall(Language,
            member(org_source_block{target:Target, language:Language}, Blocks),
            Languages0),
