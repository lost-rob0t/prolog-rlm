...
context_mount_resolve(Store, Name0, Scope0, Options, Outcome) :-
    mount_outcome(resolve,
                  context_mount_resolve_(Store, Name0, Scope0, Options),
                  Outcome).
...
context_mount_prompt(Store, Name0, Scope0, Options, Outcome) :-
    mount_outcome(prompt,
                  context_mount_prompt_(Store, Name0, Scope0, Options),
                  Outcome).
...
context_mount_runtime_reset :-
    with_mutex(rlm_context_mount,
               ( findall(Ref, cached_context_ref(Ref), Refs0),
                 sort(Refs0, Refs),
...
context_mount_(Store, Name0, Source0, Options, Binding) :-
    normalize_mount_request(Name0, Source0, Options, Request),
    mount_by_lifetime(Request.lifetime, Store, Request, Binding).

mount_by_lifetime(ephemeral, _, Request, Binding) :-
    !,
    register_descriptor(Request.source, Request.context_options, ContextRef),
    public_mount(Request, none, 1, mounted, Public),
    Binding = context_mount_binding{mount:Public, context_ref:ContextRef}.
...
persistent_publish_or_reuse(Store, Key, Request, Artifact) :-
    ...
    ;   persistent_value(Request, mounted, Value),
        Provenance = _{producer:rlm_context_mount, operation:mount},
        artifact_put(Store,
                     Namespace,
                     Key,
                     context_mount,
...
context_mount_prompt_(Store, Name0, Scope0, Options, Projection) :-
    ...
    Projection = context_mount_prompt{mount:Public,
                                      text:Text,
                                      truncated:Truncated,
                                      max_chars:MaxChars}.
