...
Projects model identity, cumulative token I/O, and an explicit observation of
current context occupancy into frontend-safe status data.  The module owns no
UI and performs no provider, model, or context work.
...
clients must not infer it from cumulative token usage.
*/
