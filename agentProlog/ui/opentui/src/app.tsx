    ...
    <box flexDirection="column" width="100%" height="100%" padding={1} gap={1}>
      <box borderStyle="rounded" padding={1} flexDirection="column">
        ...
        <For each={error() ? [error()!] : []}>
          {(message: string) => <text>{`ERROR ${message}`}</text>}
        </For>
      </box>
      ...
      <box flexDirection="row" gap={1} flexGrow={1}>
        <box borderStyle="rounded" padding={1} flexDirection="column" flexGrow={2}>
          ...
          <For each={view()?.messages ?? []}>
            {(message: JsonObject) => <text>{`${short(message.role)}: ${short(message.text)}`}</text>}
          </For>
        </box>
        ...
        <box borderStyle="rounded" padding={1} flexDirection="column" flexGrow={1}>
          <text>Activity</text>
          <For each={view()?.tools ?? []}>
            {(tool: JsonObject) => <text>{`tool ${short(tool.name)} · ${short(tool.status)}`}</text>}
          </For>
          <For each={view()?.subagents ?? []}>
            {(agent: JsonObject) => <text>{`subagent ${short(agent.id)} · ${short(agent.status)}`}</text>}
          </For>
          <For each={view()?.verification ?? []}>
            {(verification: JsonValue) => <text>{`verify ${short(verification)}`}</text>}
          </For>
        </box>
      </box>

      <For each={approval() ? [approval()!] : []}>
        {(item: JsonObject) => (
          <box borderStyle="rounded" padding={1} flexDirection="column">
      ...
      <box borderStyle="rounded" padding={1} flexDirection="column">
        ...
        <For each={(view()?.indeterminate_effects ?? []).slice(-1)}>
          {(effect: JsonValue) => <text>{`indeterminate ${short(effect)}`}</text>}
        </For>
        <For each={(view()?.traces ?? []).slice(-1)}>
          {(trace: JsonValue) => <text>{`trace ${short(trace)}`}</text>}
        </For>
        <For each={(view()?.extensions ?? []).slice(-3)}>
          {(extension: JsonValue) => <text>{`extension ${short(extension)}`}</text>}
        </For>
